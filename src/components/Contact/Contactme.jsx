...
export default function ContactMe() {
  ...
  // Auto-trigger animations when component mounts (final portfolio section)
  useEffect(() => {
    const element = containerRef.current;
    if (!element) return;

    // Small delay to allow for smooth transition from previous sections
    const initTimer = setTimeout(() => {
      element.classList.remove("opacity-0", "translate-y-8");
      element.classList.add("animate-fadeUpScale");
      ...
      animatedElements.forEach((el, index) => {
        ...
        }, index * 150); // Slightly longer delay for dramatic effect
      });
    }, 300); // Initial delay for smooth portfolio flow
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-5 overflow-hidden">
        <div 
          className="absolute top-1/4 left-1/4 w-64 h-64 bg-gradient-to-r from-green-400/20 to-blue-500/20 rounded-full blur-3xl animate-float"
          style={{
            transform: `translate(${mousePosition.x * 30}px, ${mousePosition.y * 30}px)`,
          }}
        />
        <div 
          className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-full blur-3xl animate-floatDelay"
          style={{
      ...
      <section className="relative z-20 py-8">
        <main className="relative z-10 px-4 sm:px-8 md:px-12 py-16 max-w-3xl w-full flex flex-col items-center">
          <div 
            data-animate
            className="group cursor-default opacity-0"
            onMouseEnter={(e) => {
              e.currentTarget.style.transform = `perspective(1000px) rotateX(${mousePosition.y * 5}deg) rotateY(${mousePosition.x * 5}deg)`;
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg)';
            }}
          >
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold text-white mb-2 text-center tracking-wide transition-all duration-500 group-hover:scale-110 animate-textGlow">
              Contact <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-blue-500 to-purple-600 animate-gradientShift">Me</span>
            </h1>
          </div>

          <p
            data-animate
            className="text-white/80 font-medium mb-12 text-center text-sm sm:text-base max-w-sm animate-fadeInUp opacity-0"
            style={{ 
        ...
        {/* Enhanced Footer */}
        <footer className="w-full text-center py-6 text-gray-300 bg-transparent relative z-10 animate-slideInUp opacity-0" style={{ animationDelay: "1.2s", animationFillMode: "forwards" }}>
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-gray-400 font-medium text-center animate-fadeIn" style={{ animationDelay: "1.5s" }}>
        ...
        <div className="absolute bottom-0 left-0 w-full flex justify-center pointer-events-none z-0 overflow-hidden mb-4">
          <div className="scrolling-text-container">
            <span
              className="font-black text-[clamp(3rem,12vw,12rem)] text-white/8 select-none uppercase tracking-[-0.05em] whitespace-nowrap animate-scrollTextSmooth"
              aria-hidden="true"
            >
      ...
      <style jsx>{`
        ...
        @keyframes fadeUpScale {
          from { opacity: 0; transform: translateY(60px) scale(0.9); filter: blur(6px); }
        ...
        .animate-scrollTextSmooth {
          display: inline-block;
          animation: scrollTextSmooth 20s linear infinite;
        }
        
        .animate-fadeUpScale {
          animation: fadeUpScale 1s cubic-bezier(0.16, 1, 0.3, 1) both;
        }
        ...
        html, body {
          ...
          /* If you want to force the scrollbar to the right side instead of hiding, uncomment the next line: */
          /* direction: ltr; */
        }
      `}</style>
    </div>
