  ...
  useEffect(() => {
    ...
    const displayShaderSource = `
      ...
      void main () {
          ...
          // Reduce alpha when over a button to make it more transparent
          a = isOverButton > 0.5 ? a * 0.3 : a;
    ...
    const splatShader = compileShader(
      ...
      `
        ...
        void main () {
            vec2 p = vUv - point.xy;
            p.x *= aspectRatio;
            // Reduce splat intensity when over a button
            vec3 splat = exp(-dot(p, p) / radius) * color * (isOverButton > 0.5 ? 0.3 : 1.0);
    ...
    const blit = (() => {
      ...
      return (target, clear = false) => {
        ...
        if (clear) {
          gl.clearColor(0.0, 0.0, 0.0, 0.0); // Clear to fully transparent
    ...
    function updateFrame() {
      ...
      // Check if there has been recent interaction
      const now = Date.now();
    ...
    function drawDisplay(target) {
      ...
      gl.uniform1f(displayMaterial.uniforms.isOverButton, isOverButton.current ? 1.0 : 0.0);
      blit(target);
    }
    ...
    function splat(x, y, dx, dy, color) {
      ...
      gl.uniform1f(splatProgram.uniforms.radius, correctRadius(config.SPLAT_RADIUS / 50.0));
      gl.uniform1f(splatProgram.uniforms.isOverButton, isOverButton.current ? 1.0 : 0.0);
      blit(velocity.write);
      velocity.swap();
    ...
    function generateColor() {
      // Generate shades of grey (including black)
      const shade = Math.random() * 0.5; // Range from 0 (black) to 0.5 (grey)
    ...
    // Function to check if the cursor is over a button
    function checkIfOverButton(clientX, clientY) {
    ...
    window.addEventListener('touchend', (e) => {
      ...
      isOverButton.current = false; // Reset when touch ends
    });
